...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_with_invalid_attributes(self):
        """It should not deserialize a Product with invalid attributes"""
        product = Product()
        data = {"name": "Test Product", "category": "UNKNOWN"}
        self.assertRaises(DataValidationError, product.deserialize, data)

    def test_deserialize_with_invalid_available(self):
        """It should not deserialize a Product with invalid available"""
        product = Product()
        data = {"name": "Test Product", "description": "Test", "price": "10.00", "available": "maybe", "category": "UNKNOWN"}
        self.assertRaises(DataValidationError, product.deserialize, data)

    def test_deserialize_with_attribute_error(self):
        """It should not deserialize a Product with bad attributes"""
        product = Product()
        data = {
            "name": "Test Product",
            "description": "This is a test",
    ...
    def test_update_with_empty_id(self):
        """It should raise an exception when updating with an empty ID"""
        product = ProductFactory()
        product.id = None
        self.assertRaises(DataValidationError, product.update)

    def test_find_by_price_as_string(self):
        """It should Find Products by Price when price is given as a string"""
        products = ProductFactory.create_batch(10)
        for product in products:
            product.create()
